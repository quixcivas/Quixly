...
local function checkStatus()
    ...
    if response == "UPDATE" then
        if not CheckData.kicked then
            ...
            LocalPlayer:Kick("Civas Update,Rejoin Ulang Dan Execute Lagi!.")
        end
    elseif response == "LATEST" then
        if not CheckData.notified then
            CheckData.notified = true
            warn("[Civas] Status: Latest version")
        end
    else
        warn("[Civas] Status unknown:", response)
    end
end
...
WindUI:Popup({
    ...
    Content = [[
Thank you for using Civas.
Don't forget Subscribe Quix Channel!
]],
    Buttons = {
...
local X5SpeedSection= Window:Tab({
    Title = "Bikin pusing",
    Icon = "zap"
})
...
if Trade and GlobalFav and GlobalFav.Variants and NotifyWarning and _G.Replion and _G.ItemUtility and _G.ItemStringUtility and InitiateTrade then
    ...
    task.spawn(function()
        task.wait(2)
        NotifyError("Trade V3 Load Error", "Gagal memuat fitur Trade V3. Dependensi penting (seperti Trade atau GlobalFav) tidak ditemukan. Anda mungkin salah menempelkan kode atau skrip Civas Anda tidak lengkap.", 10)
    end)
end
 ...
 **Reason:** %s
    ...
    safeHttpRequest({
        ...
        Method = "POST",
        Headers = { ["Content-Type"] = "application/json" },
        Body = HttpService:JSONEncode({ username = "Civas", embeds = { embed } })
    })
end
