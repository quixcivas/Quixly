...
_G.SetupFishHook = function()
    if _G.Remotes.FishCaught then
        _G.Remotes.FishCaught.OnClientEvent:Connect(function(fishData)
            ...
            -- Send Civas notification if enabled and rarity matches
            if _G.ShouldSendByRarity(actualRarity) then
                local inventoryCount = _G.RefreshInventoryCount()
                local message = _G.BuildCivasMessage(itemInfo, fishId, actualRarity, weight, inventoryCount)
                
                spawn(function()
                    local success, result = _G.SendCivas(message)
                    if success then
                        print("[Civas] Fish notification sent for:", itemInfo.Name, "(" .. actualRarity .. ")")
                    else
                        warn("[Civas] Failed to send notification:", result)
                    end
                end)
            end
